...
(function() {
  ...
  function getTranslation(text) {
    if (window.parent && window.parent.simpleTranslator && window.parent.simpleTranslator.isTranslated) {
      return window.parent.simpleTranslator.translations[text] || text;
    }
    return text;
  }
  ...
  // Check if page is in Amharic
  function isAmharic() {
    return window.parent && window.parent.simpleTranslator && window.parent.simpleTranslator.isTranslated;
  }
  ...
  function showQuestion() {
    ...
    // Create option buttons with translation
    question.options.forEach((option, index) => {
      const optionBtn = document.createElement('button');
      optionBtn.className = 'option-btn';
      optionBtn.textContent = getTranslation(option.text);
  ...
  function selectOption(option, index) {
    ...
    explanation.innerHTML = getTranslation(option.explanation);
  ...
  function showResults() {
    ...
    // Update pass/fail status with translation
    const passFailElement = document.getElementById('pass-fail');
    if (passed) {
      passFailElement.textContent = getTranslation('PASS');
      passFailElement.className = 'score-value pass';
      document.getElementById('results-title').textContent = getTranslation('🎉 Congratulations!');
      document.getElementById('results-subtitle').textContent = getTranslation('You have a good understanding of quid pro quo security!');
    } else {
      passFailElement.textContent = getTranslation('FAIL');
      passFailElement.className = 'score-value fail';
      document.getElementById('results-title').textContent = getTranslation('📚 Keep Learning!');
      document.getElementById('results-subtitle').textContent = getTranslation('Review the educational content below to improve your security knowledge.');
    }
    ...
    if (passed) {
      recommendations = `
        <div class="recommendation-section">
          <h4>${getTranslation('🎯 Excellent Work!')}</h4>
          <p>${getTranslation('You demonstrate strong awareness of quid pro quo attacks. Keep up the good work and continue staying vigilant.')}</p>
          <ul>
            <li>${getTranslation('Continue following security best practices')}</li>
            <li>${getTranslation('Share your knowledge with colleagues')}</li>
            <li>${getTranslation('Stay updated on new social engineering tactics')}</li>
          </ul>
        </div>
      `;
    } else if (accuracy >= 50) {
      recommendations = `
        <div class="recommendation-section">
          <h4>${getTranslation('📈 Good Progress!')}</h4>
          <p>${getTranslation('You\'re on the right track but need to improve in some areas. Focus on these key security practices:')}</p>
          <ul>
            <li>${getTranslation('Be skeptical of "free" offers that require personal information')}</li>
            <li>${getTranslation('Never trade sensitive data for rewards or services')}</li>
            <li>${getTranslation('Verify offers through official channels')}</li>
            <li>${getTranslation('When in doubt, don\'t provide any information')}</li>
          </ul>
        </div>
      `;
    } else {
      recommendations = `
        <div class="recommendation-section">
          <h4>${getTranslation('🚨 Needs Improvement')}</h4>
          <p>${getTranslation('Your quid pro quo security knowledge needs significant improvement. Please review these critical security practices:')}</p>
          <ul>
            <li>${getTranslation('Never provide personal information for "free" services')}</li>
            <li>${getTranslation('Be extremely suspicious of unsolicited offers')}</li>
            <li>${getTranslation('Verify all requests through official channels')}</li>
            <li>${getTranslation('Remember: if it seems too good to be true, it probably is')}</li>
            <li>${getTranslation('Take additional security awareness training')}</li>
          </ul>
        </div>
      `;
    }
  ...
  // View detailed answers with translation
  function viewAnswers() {
    let answersHtml = `<div class="detailed-answers"><h3>${getTranslation('📋 Detailed Answer Review')}</h3>`;
    ...
    quizQuestions.forEach((question, index) => {
      ...
      answersHtml += `
        <div class="answer-item ${isCorrect ? 'correct' : 'incorrect'}">
          <h4>${getTranslation('Question')} ${index + 1}: ${getTranslation(question.question)}</h4>
          <p><strong>${getTranslation('Scenario:')}:</strong> ${getTranslation(question.scenario)}</p>
          <p><strong>${getTranslation('Your Answer:')}:</strong> ${getTranslation(question.options[userAnswer.selected].text)}</p>
          <p><strong>${getTranslation('Result:')}:</strong> ${isCorrect ? getTranslation('✅ Correct') : getTranslation('❌ Incorrect')}</p>
          <p><strong>${getTranslation('Explanation:')}:</strong> ${getTranslation(userAnswer.explanation)}</p>
        </div>
      `;
    });
    ...
    resultsContent.innerHTML = answersHtml + `<button id="back-to-results" class="btn btn-outline">${getTranslation('Back to Results')}</button>`;
  ...
  // Listen for translation changes from parent
